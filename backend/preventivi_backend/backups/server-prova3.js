  ...
  try {
    ...
    const [rows] = await connection.query(`
      ...
      WHERE
        p.id_preventivo = ?
    `, [preventivoId]);
    console.log("Query Results:", rows); // Aggiungi questo log
    ...
    const font = await pdfDoc.embedFont(StandardFonts.Helvetica); // Usa StandardFonts.Helvetica
    ...
    const pdfPath = path.join(__dirname, 'preventivo.pdf');
    fs.writeFileSync(pdfPath, pdfBytes);

    console.log(`PDF salvato con successo in: ${pdfPath}`);
    ...
    console.error(error);
   console.error('Errore durante la generazione del PDF:', error); // Log dettagliato
