...
app.post('/preventivo', async (req, res) => {
  const { data_preventivo, id_cliente, prezzo_totale, lavori } = req.body;  // `lavori` è un array di oggetti lavoro
  ...
  try {
    ...
    if (clientResult.length === 0) {
      const { nome, cognome, città, via } = req.body.cliente; // supponiamo che i dati del cliente siano nel corpo della richiesta
      const [insertClientResult] = await connection.query(
        'INSERT INTO Cliente (nome, cognome, città, via) VALUES (?, ?, ?, ?)',
    ...
    for (const lavoro of lavori) {
      const { tipo_lavoro, descrizione_lavoro, prezzo, caldaia, data_caldaia } = lavoro;
      await connection.query(
        'INSERT INTO Lavoro (id_preventivo, tipo_lavoro, descrizione_lavoro, prezzo, caldaia, data_caldaia) VALUES (?, ?, ?, ?, ?, ?)',
        [id_preventivo, tipo_lavoro, descrizione_lavoro, prezzo, caldaia, data_caldaia]
      );
    }
