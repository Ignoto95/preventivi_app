...
app.get('/generate-pdf/:id', async (req, res) => {
  ...
  try {
    ...
    preventivo.lavori.forEach(lavoro => {
      ...
      yPosition -= 20;
      page.drawText(`Prezzo: €${lavoro.prezzo.toFixed(2)}`, { x: 50, y: yPosition, size: 12, font });
      yPosition -= 30;
    });

    // Aggiungi il prezzo totale
    page.drawText(`Prezzo totale: €${preventivo.prezzo_totale.toFixed(2)}`, { x: 50, y: yPosition, size: 14, font });
